...
- [ ] **Turn the weekly schedule back on.** Still commented out in `ingest.yml` as of 2026-08-31. Deliberate: the judging pipeline, a threshold calibrated against exactly one week of data, and publish-on-merge all landed at once, and the threshold is the one most likely to need moving. Run `workflow_dispatch` with the dry-run input a couple of times, check the near-miss table for papers that should have shipped, then uncomment the cron. It was Monday 07:00 UTC.
- [x] **Scheduled trigger paused (2026-08-10)** — the `schedule:` block in [ingest.yml](../.github/workflows/ingest.yml) is commented out while Rob keeps tuning the search algorithm, so an unattended Monday run doesn't open a PR mid-iteration. `workflow_dispatch` (manual trigger) is still live, so test runs still work. Uncomment the schedule to resume the weekly automatic run — it was Monday 07:00 UTC (still needs confirming as the actual preferred time once resumed).
- [x] **Word-count and theme/tag confusion, both addressed 2026-08-31.** An over-length finding is now re-asked once before writing (`shortenFinding` in `extract.mjs`), with `validate-entries.mjs`'s hard cap kept as the backstop. Tag values misfiled under `secondary_themes` are moved to `tags` rather than skipping the paper — the same hand-fix applied across PR #4, now automatic and reported in the PR. A tag value as the *primary* theme still fails, deliberately: that means no theme was extracted, and guessing at the field the site is organised by is worse than skipping. `needs-review` on any closed-vocabulary field is now a clean skip rather than a file that breaks the build.
- [ ] Original note, kept for context: replace or fine-tune the word-count example fix — real improvement (30% -> 18% of findings over the 40-word cap across two live runs) but not zero. Consider the same fix for the recurring theme/tag confusion (DeepSeek keeps putting tag-like values — `trust-calibration`, `evaluation-methods`, `memory-and-context` — into `theme`/`secondary_themes` instead of `tags`; ~6 candidates skipped for this in each of the last two runs, same root cause as the word-count issue).
...
- [x] The ingest workflow has now run for real multiple times (PRs #1 and #2, both closed as superseded during search tuning; PR #3 is the current one, open, `status: queued`) — `npm run ready` is the way to see what's queued before flipping any to `published`.
...
- ~~**Scored/ranked retrieval, semantic judging, and a weekly digest**~~ — **built 2026-08-31**, on Rob's explicit call, after he described the manual ChatGPT process he had been running for a year. See `scripts/lib/judge.mjs` and the rewritten `PIPELINE.md`. What was built: triage then scoring on title/abstract, selection in code (never in the model), a calibrated threshold, a synthesis in the PR body, and a near-miss table so culls stay visible. What was avoided, and still should be: the invented numeric weights. Weights are equal, and the threshold was measured against a real week rather than guessed. The original reasoning is kept below because the objection was sound and the answer to it is now part of the design.

  Two things that only showed up once it ran live, both now fixed and worth remembering: arXiv cross-listing duplicated 23% of a run's candidates (nothing deduplicated within a run, only against existing entries), and arXiv 429s silently cost an entire category's papers for the week.

  The original 2026-08-10 note follows:
